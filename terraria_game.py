...
pygame.display.set_caption("Alex & Zofia Present: Terraria")
...
def main():
    ...
    prevCameraX, cameraX, cameraY, yCamOffset = 2, 2, 0, -10
    gameSeed = 4
    ...
    while Running:
        ...
        trader.menu.draw(screen, HEADER_FONT, FONT2)
        blockMenu.draw(screen, FONT2)

        if pygame.mouse.get_pressed()[0]:
            if not trader.menu.isActive and not inventory.menu.isActive:
                if break_block(mouse, player, cameraX, inventory, terrain, yCamOffset, screen, INSTRUCTIONS_FONT):
                    shaderUpToDate = False
