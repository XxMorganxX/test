...
class BlockMenu(menu.Menu):
    ...
    def draw(self, window, font):
        if self.isActive:
            self.rects = {}
            pygame.draw.circle(window, env.BLACK, (env.WINDOW_WIDTH/2, env.WINDOW_HEIGHT/2), self.outerRadius)
            pygame.draw.circle(window, env.BROWN_MENU, (env.WINDOW_WIDTH/2, env.WINDOW_HEIGHT/2), self.outerRadius-4)
            ...
            for block in self.blocks:
                ...
                xPos = env.WINDOW_WIDTH/2 + int((self.outerRadius-90) * math.cos(angle)) - (self.width/2)
                yPos = env.WINDOW_HEIGHT/2 + int((self.outerRadius-90) * math.sin(angle)) - (self.height/2)

                rect = pygame.Rect(xPos, yPos, self.width, self.height)
...
class Trader(pygame.sprite.Sprite):
    def __init__(self, terrain, yOffset):
        ...
        self.item_name_price = {"Grass":0.1, "Dirt":0.1, "Stone":3, "Metal":6, "Diamond": 45, "Garnet": 75, "Topaz": 70, "Alzo": 120}
    ...
    def upgrade(self, inventory, player, clicker):
        if self.upgradeClicked != None:
            ...
            if self.upgradeClicked <= 4:
                ...
                # Upgrade conditions for mining speed
                ...
                # Upgrade conditions for storage level
            ...
            if self.upgradeClicked == 8 and self.inventoryPrice != 0:
                ...
                inventory.storage = []
                self.activateMenu(inventory.storage)
    ...
    def draw(self, window, headerFont, font):
        ...
        if super().draw(window):
            ...
            for i, (upgrade, level) in enumerate(self.mining_upgrades.items()):
                ...
                self.textRects.append(pygame.Rect((self.rect[0]+(self.width/4))-(text.get_size()[0]/2), ((headerSize[0])+ self.top + ((30*2)*i)), *text.get_size()))
                window.blit(text, ((self.rect[0]+(self.width/4))-(text.get_size()[0]/2), ((headerSize[0])+ self.top + ((30*2)*i))))            
...
- mining level = which blocks youre able to break x3
- attack speed x3
- attack damage x3
- Fortune x3
- Carrying Capacity
